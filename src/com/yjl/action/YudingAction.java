	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (hao != null) {
			if (dbm.isDing(hao) != null) {
				out.println("<script>alert('你的车已经停在车位上！');window.location.href='chewei/tlist.jsp'</script>");
			} else {
				java.text.SimpleDateFormat format = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				String sql = "update chewei set chepai='" + hao + "',adate='" + format.format(new java.util.Date()) + "' where id=" + id;
